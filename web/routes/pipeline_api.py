            ...
            if text.startswith("__PIPELINE_SUMMARY__:"):
                try:
                    ...
                    async with _state_lock:
                        ...
                        _task_status[task_id]["track_memory"] = summary.get("track_memory", [])
                        _task_status[task_id]["event_log_tail"] = summary.get("event_log_tail", [])
                        _task_status[task_id]["agent_trace"] = summary.get("agent_trace", [])
                except Exception:
                    pass
...
async def get_pipeline_memory(task_id: str):
    task = _task_status.get(task_id)
    if not task:
        raise HTTPException(status_code=404, detail="任务不存在")
    return {
        "task_id": task_id,
        "track_memory": task.get("track_memory", []),
        "event_log": task.get("event_log_tail", []),
        "agent_trace": task.get("agent_trace", []),
    }
