...
function onCameraSourceChange() {
  ...
  if (urlInput) {
    ...
    if (val === 'rtsp') {
      urlInput.placeholder = 'rtsp://127.0.0.1/stream';
    } else if (val === 'custom') {
...
function renderPaperSummary(data) {
  ...
  if (!box || !data) return;
  let summary = document.getElementById('paperSummaryBox');
  if (!summary) {
    summary = document.createElement('div');
    summary.id = 'paperSummaryBox';
    summary.style.cssText = 'margin-top:8px;font-size:12px;color:#475569;line-height:1.6';
    box.appendChild(summary);
  }
  const parts = [];
  if (data.exact_matches != null) parts.push(`exact=${data.exact_matches}`);
  if (data.high_risk_tracks != null) parts.push(`high-risk=${data.high_risk_tracks}`);
  if (data.medium_risk_tracks != null) parts.push(`mid-risk=${data.medium_risk_tracks}`);
  if (data.avg_uncertainty != null) parts.push(`uncertainty=${data.avg_uncertainty}`);
  summary.textContent = parts.length ? `Agent summary: ${parts.join(' | ')}` : '';
}
