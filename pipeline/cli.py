...
用法:
    ...
    默认使用中央 VLM 工具控制器；可通过 controller_mode 选择 legacy 三步链路。
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--visual-backend", choices=["dinov2", "qwen_vl_reranker"], default=None, help="启用指定的视觉档案检索服务")
    parser.add_argument("--dinov2-repo", default=None, help="本地 DINOv2 代码目录")
    parser.add_argument("--dinov2-weights", default=None, help="本地 DINOv2 权重文件")
    ...
    parser.add_argument("--controller-mode", choices=["legacy", "central_vlm", "mock"], default=None, help="实体控制模式（默认沿用配置）")
...
def _merge_args_to_config(args, config: dict) -> dict:
    ...
    if getattr(args, "visual_backend", None):
        visual.update(enabled=True, backend=args.visual_backend)
    if getattr(args, "dinov2_repo", None):
        visual["model_repo"] = args.dinov2_repo
    if getattr(args, "dinov2_weights", None):
        visual["weights"] = args.dinov2_weights
    ...
    if args.controller_mode is not None:
        config.setdefault("experiment", {})["controller_mode"] = args.controller_mode
    return config
...
def _print_final_results(stats: dict) -> None:
    """Print one compact final record for each tracked entity."""
    memories = stats.get("track_memory", []) or []
    console.print("\n[bold]最终识别结果[/bold]")
    if not memories:
        console.print("[yellow]未形成有效实体结果[/yellow]")
        return
    table = Table()
    ...
    for item in memories:
        if not isinstance(item, dict):
            continue
        reason_values = item.get("review_reasons", []) or []
        reason = "; ".join(str(value) for value in reason_values) or "-"
        identity = str(item.get("verified_identity", "") or item.get("archive_candidate_id", "") or "-")
        table.add_row(
            str(item.get("entity_id", "-")),
            ",".join(str(value) for value in item.get("member_track_ids", []) or []) or "-",
            identity,
            str(item.get("identity_state", "unknown")),
            str(item.get("identity_evidence_source", "none")),
            reason,
        )
    console.print(table)
...
def main() -> None:
    ...
    try:
        ...
        for key, value in stats.items():
            if key in {"track_memory", "event_log_tail", "agent_trace", "paper_summary"}:
                continue
            table.add_row(key.replace("_", " ").title(), str(value))
        console.print(table)
        _print_final_results(stats)
