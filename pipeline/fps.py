...
class LatencyMeter:
    ...
    def _summarize_values(values: list[float]) -> dict[str, float]:
        ...
        return {
            ...
            "p50": round(ordered[count // 2], 3),
            "p95": round(ordered[min(int(count * 0.95), count - 1)], 3),
    ...
    def get_all_time_stats(self) -> dict[str, dict[str, float]]:
        return {ch: self._summarize_values(values) for ch, values in self._lifetime_samples.items()}

    def reset(self, channel: str | None = None) -> None:
        if channel:
            self._samples.pop(channel, None)
            self._lifetime_samples.pop(channel, None)
        else:
            self._samples.clear()
