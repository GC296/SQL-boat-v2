...
class ShipDetector:
    ...
    def __init__(
        ...
        self._tracker_tmp_file: str | None = None
        self._model = None
        try:
            from ultralytics import YOLO
        except ImportError as exc:
            raise RuntimeError(
                "Ultralytics is not installed. Install the project dependencies with "
    ...
    def cleanup(self) -> None:
        tracker_tmp_file = getattr(self, "_tracker_tmp_file", None)
        if tracker_tmp_file:
            try:
                Path(tracker_tmp_file).unlink(missing_ok=True)
            except Exception:
                pass
